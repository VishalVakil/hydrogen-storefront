...
export function Reviews({ productId, reviews }) {
    ...
    useEffect(() => {
        // Check if reviews is a promise and resolve it
        if (reviews && reviews.then) {
            reviews.then((data) => {
                setResolvedReviews(data || []);
            });
        } else {
            // If reviews is already an array, set it directly
            setResolvedReviews(reviews || []);
        }
    }, [reviews]);

    const averageRating = resolvedReviews.length
        ? resolvedReviews.reduce((acc, review) => acc + review.rating, 0) /
        resolvedReviews.length
        : 0;

    const sortedReviews = [...resolvedReviews].sort((a, b) => {
        switch (sortBy) {
            case 'highest':
    ...
    if (!resolvedReviews.length) {
        return (
            <div style={{ padding: '32px 0' }}>
                ...
                <button onClick={() => setShowReviewForm(true)} style={commonButtonStyles}>
                    Write a Review
                </button>
            </div>
        );
        ...
        <section style={{ padding: '32px 0' }}>
            ...
            >
                <div>
                    ...
                    >
                        Based on {resolvedReviews.length} review
                        {resolvedReviews.length !== 1 ? 's' : ''}
                    </p>
                </div>
